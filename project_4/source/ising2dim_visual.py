...
def task_e(temperatures=(1, 2.4)):
    ...
    for size in [40, 60, 80, 100]:
        ...
        for spin_matrix in ['ordered', ] : # 'random', ]:  #
            print spin_matrix
            if plotting:
                df = pd.read_csv('task_e_transition_size{}_{}.csv'.format(size, spin_matrix))
                plt.figure()
                for i in [0, 1, 4]:
                    plt.plot(temperatures, df.values[:, i+1], label=names[i])

                plt.title('Size={}, {} spin matrix'.format(size, spin_matrix))
                # plt.ylabel('Probability $P(E/N)$')
                plt.xlabel('Temperature $[k_b K / J]$')
                plt.legend(framealpha=0.3)
                plt.savefig('task_e_phase_size{}_{}.png'.format(size, spin_matrix))
            else:
                data = []
                for temperature in temperatures:
                    print temperature
                ...
                df.to_csv('task_e_transition_size{}_{}.csv'.format(size, spin_matrix))
...
#     print energy_variance2(T)/energy_variance(T)
    ...
    task_e(temperatures=np.linspace(2, 2.3, 7))
    plt.show('hold')
    # Main program
